...
spaghetti_annual_region_encounters_pre <- annual_region_encounters %>%
  rename(
    Year = `Fiscal\r\nYear`,    # Rename column with newline characters to "Year"
    Region = `Region`, 
    Encounters = `Quantity` # Rename "Quantity" to "Encounters" for better readability
  ) %>%
  filter(Year >= 2004 & Year <= 2025) # Keep only data from 2004 to 2025

# Check how many unique regions are present in the dataset
length(unique(annual_region_encounters$Region))

# Identify top regions based on total encounters
top_regions <- spaghetti_annual_region_encounters_pre %>%
  group_by(Region) %>%
  summarise(Total = sum(Encounters)) %>% # Sum encounters by region
  arrange(desc(Total)) # Sort regions by descending encounter total

# Filter the original preprocessed data to include only the top regions
...
spaghetti_annual_region_encounters_pre <- annual_region_encounters %>%
  ...
  filter(Year >= 2014 & Year <= 2025) # Keep only data from 2014 to 2025

# Count the number of unique regions in the original dataset
length(unique(annual_region_encounters$Region))

# Identify the top 5 regions based on total encounters over the selected years
top_regions <- spaghetti_annual_region_encounters_pre %>%
  group_by(Region) %>%
  summarise(Total = sum(Encounters)) %>% # Calculate total encounters per region
  arrange(desc(Total)) %>% # Sort from highest to lowest total
  slice_head(n = 5) # Select only the top 5 regions

# Filter the data to keep only the top 5 regions
spaghetti_annual_region_encounters_filtered_pre <- spaghetti_annual_region_encounters_pre %>%
  filter(Region %in% top_regions$Region)

# Print the filtered data to visually confirm the result
print(spaghetti_annual_region_encounters_filtered_pre)
ggplot(spaghetti_annual_region_encounters_pre, aes(x = Year, y = Encounters, color = Region)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
...
ggplot(full_data, aes(x = Year, y = Encounters, color = Region)) +
  ...
  geom_vline(xintercept = 2004, linetype = "dashed", color = "green") +
  geom_vline(xintercept = 2012, linetype = "dashed", color = "blue") +
  geom_vline(xintercept = 2015, linetype = "dashed", color = "orange") +
  geom_vline(xintercept = 2020, linetype = "dashed", color = "pink") +
  geom_vline(xintercept = 2023, linetype = "dashed", color = "purple") +
  ...
  labs(
    ...
    y = "Encounters" # Label for y-axis
  ) +
  theme_minimal() + # Clean and modern visual theme
  theme(
    legend.position = "right", # Show legend on the right
    plot.title = element_text(face = "bold", size = 14) # Make title bold and prominent
  )
